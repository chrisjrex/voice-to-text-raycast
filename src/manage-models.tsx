...
function downloadStatusTag(
  downloading: boolean,
  downloaded: boolean,
): List.Item.Accessory {
  if (downloading)
    return { tag: { value: "Downloading...", color: Color.Blue } };
...
async function confirmDelete(
  ...
  try {
    ...
    await showToast({
      ...
      title: `Deleted ${title.replace(/^Delete /, "").replace(/\?$/, "")}`,
    });
    return true;
  } catch (err) {
    await showToast({
...
export default function Command() {
  ...
  const { statuses: piperStatuses, refresh: refreshPiper } =
    useTtsVoiceStatus();
  const [activePiperVoice, setActivePiperVoiceState] = useState<
    string | undefined
  >();
  const [piperEngineInstalled, setPiperEngineInstalled] = useState<
    boolean | null
  >(null);
  const piperDownloader = useDownloader();

  const [activeKokoroVoice, setActiveKokoroVoiceState] = useState<
    string | undefined
  >();
  const [kokoroBaseDownloaded, setKokoroBaseDownloaded] = useState(() =>
    isKokoroModelDownloaded(),
  );
  const [kokoroEngineInstalled, setKokoroEngineInstalled] = useState<
    boolean | null
  >(null);
  const [kokoroVoiceStatuses, setKokoroVoiceStatuses] = useState<
  ...
  const [activeSystemVoice, setActiveSystemVoiceState] = useState<
    string | undefined
  >();

  useEffect(() => {
    isPythonPackageInstalled(pythonPath, "piper").then(setPiperEngineInstalled);
    isPythonPackageInstalled(kokoroPython, "kokoro").then(
      setKokoroEngineInstalled,
    );
  ...
  async function handleSetActiveStt(modelValue: string) {
    ...
    await showToast({
      style: Toast.Style.Success,
      title: `Active model: ${model?.title ?? modelValue}`,
    });
  }
  ...
  async function handleSetActivePiper(voiceId: string) {
    ...
    await showToast({
      style: Toast.Style.Success,
      title: `Active voice: ${voice?.title ?? voiceId}`,
    });
  }
  ...
  async function handleDownloadKokoroVoice(voiceId: string) {
    ...
    if (!kokoroEngineInstalled) {
      const toast = await showToast({
        style: Toast.Style.Animated,
        title: "Installing Kokoro packages...",
      });
      try {
    ...
    const label = kokoroBaseDownloaded
      ? `Kokoro voice ${voiceId}`
      : `Kokoro engine + ${voiceId}`;
  ...
  function handlePreviewSample(voiceId: string) {
    ...
    const player = spawn("afplay", [samplePath], {
      detached: true,
      stdio: "ignore",
    });
  ...
  function handlePreviewSystem(voiceId: string) {
    stopPreview();
    const player = spawn("say", ["-v", voiceId, "Testing 1, 2, 3"], {
      detached: true,
      stdio: "ignore",
    });
    activePreview = player;
  ...
  return (
    <List>
          ...
          <ModelListItem
            ...
            onDelete={async () => {
              ...
              if (
                await confirmDelete(
                  `Delete ${modelId}?`,
                  "This will remove the cached model files from disk.",
                  () =>
                    rmSync(modelCacheDir(modelId), {
                      recursive: true,
                      force: true,
                    }),
                )
      ...
      {prefs.enablePiper && (
        <List.Section
          title="Piper Voices"
          subtitle="Fast, lightweight, CPU-only TTS"
        >
          {TTS_VOICES.map((v) => (
            <ModelListItem
              key={`piper-${v.id}`}
              ...
              onDelete={async () => {
                if (
                  await confirmDelete(
                    `Delete ${v.id}?`,
                    "This will remove the voice model files from disk.",
                    () => {
                  ...
                  const anyRemaining = TTS_VOICES.some(
                    (other) =>
                      other.id !== v.id && isTtsVoiceDownloaded(other.id),
                  );
                  if (!anyRemaining && piperEngineInstalled) {
                    const uninstall = await confirmAlert({
                      title: "Uninstall Piper Voice Engine?",
                      message:
                        "No Piper voices remain. Would you also like to uninstall the voice engine (~24MB)?",
                      primaryAction: {
                        title: "Uninstall",
      ...
      {prefs.enableKokoro && (
        <List.Section title="Kokoro Voices" subtitle="High-quality neural TTS">
          {KOKORO_VOICES.map((v) => {
            ...
            const isDownloading =
              kokoroDownloader.downloadingId === `kokoro-${v.id}`;
            return (
              <ModelListItem
                key={`kokoro-${v.id}`}
                itemKey={`kokoro-${v.id}`}
                ...
                onDelete={async () => {
                  ...
                  if (
                    await confirmDelete(
                      `Delete ${voice?.title ?? v.id}?`,
                      "This will remove the voice file from disk.",
                      () => deleteKokoroVoice(v.id),
                    )
                  ) {
                    ...
                    if (!anyRemaining && kokoroBaseDownloaded) {
                      const deleteEngine = await confirmAlert({
                        ...
                        message:
                          "No Kokoro voices remain. Would you also like to uninstall the voice engine (~362MB)?",
                        primaryAction: {
                          title: "Uninstall",
                          style: Alert.ActionStyle.Destructive,
                        },
                      ...
                      if (deleteEngine) {
                        rmSync(modelCacheDir(KOKORO_MODEL_ID), {
                          recursive: true,
                          force: true,
                        });
      ...
      <List.Section
        title="System Voices"
        subtitle="macOS built-in voices for Read Aloud"
      >
        {SYSTEM_VOICES.map((v) => (
          <ModelListItem
            key={`system-${v.id}`}
